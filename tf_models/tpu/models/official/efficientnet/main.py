...
flags.DEFINE_bool(
    'use_fs', False, 'Use use few spikes coding'
)
...
def model_fn(features, labels, mode, params):
  ...
  """
    ...
    if FLAGS.use_relu:
        override_params['relu_fn'] = tf.nn.relu
    if FLAGS.use_fs:
        replace_sigmoid_with_fs()
        override_params['relu_fn'] = fs_swish
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      ...
      """
        ...
        eval_metrics = {'top_1_accuracy': metric_fn(labels, logits)}
    ...
    return tf.estimator.EstimatorSpec(  # used to be tf.estimator.tpu.TPUEstimatorSpec
        mode=mode,
        loss=loss,
        train_op=train_op,
