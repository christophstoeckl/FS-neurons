...
flags.DEFINE_string(
    'local_scratch_dir', "/calc/SHARED/imagenet", 'Scratch directory path for temporary files.')
flags.DEFINE_string(
    'raw_data_dir', None, 'Directory path for raw Imagenet dataset. '
                          'Should have train and validation subdirectories inside it.')
flags.DEFINE_string(
    'imagenet_username', "randomusername", 'Username for Imagenet.org account')
flags.DEFINE_string(
    'imagenet_access_key', "f0b059dbae8c5c6f23d9304e0818d248ae38002e", 'Access Key for Imagenet.org account')
...
def _convert_to_example(filename, image_buffer, label, synset, height, width):
  ...
  """
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
        ...
        'image/class/synset': _bytes_feature(bytes(synset, encoding='utf8')),
        'image/format': _bytes_feature(image_format),
        'image/filename': _bytes_feature(bytes(os.path.basename(filename), encoding='utf8')),
